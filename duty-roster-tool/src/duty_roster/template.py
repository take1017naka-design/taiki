...
def build_config_text(names: list[str]) -> str:
    ...
    lines += [
        ...
        "  # 予備の回数は quota_ignore を除く全員で均等にする",
        "  even_quota: true",
        ...
        f"  friday_excluded: {_yaml_list([names[ANCHOR_INDEX]] if positional else [])}",
        "  # 土曜の予備はその日の出勤者から選ぶ",
        "  saturday_requires_working: true",
        "  # 祝日に予備へ入れたら「要相談」として確認事項に出す人",
        f"  holiday_consult: {_yaml_list([names[ANCHOR_INDEX]] if positional else [])}",
